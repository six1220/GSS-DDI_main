...
class update_e(torch.nn.Module):
    ...
    def forward(self, v, dist, dist_emb, edge_index):
        j, _ = edge_index
        C = 0.5 * (torch.cos(dist * PI / self.cutoff) + 1.0)  # 位置编码？表示边的角度？表示权重？
...
class update_v(torch.nn.Module):  # 得出节点embed
    ...
    def forward(self, v, e, edge_index):
        _, i = edge_index
        out = scatter(e, i, dim=0)  # 把节点周围的边emb加和形成节点emb
        out = self.lin1(out)
        out = self.act(out)
        out = self.lin2(out)
        return v + out  # 和自己加和，获得节点本身信息


class update_u(torch.nn.Module):  # 得出全局embed
    ...
    def forward(self, v, batch):
        ...
        v = self.act(v)
        v = self.lin2(v)
        u = scatter(v, batch, dim=0)  # 将两张图中的所有节点emb加和形成图emb
        return u, v
